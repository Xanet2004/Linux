...
options {
	...
	listen-on { 127.0.0.1; 172.18.4.4; 192.168.2.4; };
	listen-on-v6 { none; };
	...
	allow-query { 127.0.0.1; 172.18.4.0/22; 192.168.2.0/25; };
...
```powershell title="liburutegia - named.conf.options"
options {
    directory "/var/cache/bind";

    listen-on { localhost; 172.18.4.5; };
    listen-on-v6 { none; };
    ...
    recursion yes;
    allow-recursion { 127.0.0.1; 172.18.4.0/22; };
    allow-query { 127.0.0.1; 172.18.4.0/22; };

    dnssec-validation auto;
};
```

```powershell title="musakola - named.conf.options"
options {
    directory "/var/cache/bind";

    listen-on { localhost; 192.168.2.5; };
    listen-on-v6 { none; };
    ...
    recursion yes;
    allow-recursion { 127.0.0.1; 192.168.2.0/25; };
    allow-query { localhost; 192.168.2.0/25; };
...
acl acl-udaletxea { 127.0.0.1; 172.18.4.0/22; };
acl acl-kiroldegia { 127.0.0.1; 192.168.2.0/25; };
...
view "bista-udaletxea" {
    ...
    zone "arrasate.eus" {
        ...
        allow-transfer { 172.18.4.5; };
        notify yes;
    };

    // reverses para la parte Udaletxea (se puede declarar por /24)
    zone "4.18.172.in-addr.arpa" IN {
        type master;
        file "/etc/bind/db.172.18.4";
        allow-transfer { 172.18.4.5; };
    };

    zone "5.18.172.in-addr.arpa" IN {
        type master;
        file "/etc/bind/db.172.18.5";
        allow-transfer { 172.18.4.5; };
    };

    zone "6.18.172.in-addr.arpa" IN {
        type master;
        file "/etc/bind/db.172.18.6";
        allow-transfer { 172.18.4.5; };
    };
};
...
view "bista-kiroldegia" {
    ...
    zone "arrasate.eus" {
        type master;
        file "/etc/bind/arrasate.eus.db.bista-kiroldegia";
        allow-transfer { 192.168.2.5; };
        notify yes;
    };

    // reverse para 192.168.2.0/25
...
zone "arrasate.eus" {
    type slave;
    file "/var/cache/bind/arrasate.eus.db.bista-udaletxea";
    masters { 172.18.4.4; };   // udala master (IP alcanzable desde liburutegia)
};

zone "4.18.172.in-addr.arpa" {
    type slave;
    file "/var/cache/bind/db.172.18.4";
    masters { 172.18.4.4; };
};

zone "5.18.172.in-addr.arpa" {
    type slave;
    file "/var/cache/bind/db.172.18.5";
    masters { 172.18.4.4; };
};

zone "6.18.172.in-addr.arpa" {
    type slave;
    file "/var/cache/bind/db.172.18.6";
    masters { 172.18.4.4; };
};
```
...
zone "arrasate.eus" {
    type slave;
    file "/var/cache/bind/arrasate.eus.db.bista-kiroldegia";
    masters { 192.168.2.4; };   // udala master en la red kiroldegia
};
...
```powershell title="udala - /etc/bind/arrasate.eus.db.bista-udaletxea"
$TTL 86400
@   IN  SOA udala.arrasate.eus. admin.arrasate.eus. (
        1
        604800
        86400
        2419200
...
```
```powershell title="udala - /etc/bind/arrasate.eus.db.bista-kiroldegia"
$TTL 86400
@   IN  SOA udala.arrasate.eus. admin.arrasate.eus. (
        1
        604800
