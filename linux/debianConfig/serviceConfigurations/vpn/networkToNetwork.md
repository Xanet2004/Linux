...
VPN connection between a computer and a network or router computer.
Encrypted.

> Note
> The only thing that changes comparing it to computerToNetwork is the file: /etc/openvpn/server/konfigurazioa.conf
> push "route 172.16.0.0 255.255.0.0"  # Network address and subnet mask pushed to clients
> route 192.168.1.0 255.255.255.0  # IP address and masc  shared by client
> client-config-dir /etc/openvpn/ccd # Folder to save each configuration of each client
## Example structure
...
tls-server                                 # Enables TLS and performs server-side TLS handshake
...
```powershell title="client - create folder"
mkdir /etc/openvpn/ccd
```

```powershell title="client - /etc/openvpn/ccd/user"
...
This VPN configuration is valid for a server that acts as router.
To achieve this, follow this guide: [routing](/linux/debianConfig/serviceConfigurations/routing.md)